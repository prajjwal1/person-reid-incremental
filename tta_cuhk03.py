...
def test(model, queryloader,galleryloader,ranks=[1,5,10,20]):
    ...
    with torch.no_grad():
        ...
        for batch,(imgs,pids,camids) in enumerate(queryloader):
            ...
            features,k = model(imgs)
            
            batch_time.update(time.time()-end)

            output_fc = "fc1"       
            fc1 = features[output_fc]
            fc1 = fc1.data.cpu()  
...
SAVED_MODEL_PATH = 'saved_models/pr.pth.tar'
...
for epoch in range(start_epoch,num_epochs) :
    ...
    if (epoch == 300 or epoch == 500 or epoch == 700 or epoch == 900 or epoch == 1200 or epoch == 1500):
